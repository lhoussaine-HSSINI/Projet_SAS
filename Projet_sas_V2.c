...
char t_n[100][30], t_p[100][30], test_cin[1][30], t_cin[100][30];
double flos, t_tre_montant[100];
...
{
    if (t_tre_montant[pos] >= flos)
    {
        t_tre_montant[pos] = t_tre_montant[pos] - flos;
        printf("L'operation est bien effectue et vous avez sortir  : %lf  LE  reste EST %lf \n", flos, t_tre_montant[pos]);
    }
    else
    {
...
void POAD(double T_1[], int N, int POO)
{
    int i, j;
    double k;
    for (i = 0; i < N - 1; i++)
        ...
        for (j = i + 1; j < N; j++)
        {
            if (POO == 1)
            {
                if (T_1[i] > T_1[j])
                {
                    ...
                    //prenom
                    strcpy(test_cin[0], t_p[i]);
                    strcpy(t_p[i], t_p[j]);
                    strcpy(t_p[j], test_cin[0]);
                    //cin
                    strcpy(test_cin[0], t_cin[i]);
                    strcpy(t_cin[i], t_cin[j]);
                    strcpy(t_cin[j], test_cin[0]);
                }
            }
            else if (POO == -1)
            {
...
void POALCBMS(double t_tre_montant[], int n, double te_mon)
{
    POAD(t_tre_montant, n, 1);
    ...
    if (po_s != -1)
    {
        ...
        {
            printf("\nNOM :\t%s", t_n[i]);
            printf("\tPRENOM :\t%s", t_p[i]);
            printf("\tCIN :\t%s", t_cin[i]);
            printf("\tMontant :\t%lf \n", t_tre_montant[i]);
        }
    }
    else
...
void PODLCBMS(double t_tre_montant[], int n, double te_mon)
{
    POAD(t_tre_montant, n, -1);
    po_s = inter(t_tre_montant, n, te_mon);
    int i;
...
void Affichage()
{
    ...
    case 3:
        ...
        scanf("%lf", &test_mon);
        POALCBMS(t_tre_montant, n, test_mon);
        break;
    case 4:
        printf("Entrez le montant : \t ");
        scanf("%lf", &test_mon);
        PODLCBMS(t_tre_montant, n, test_mon);
        break;
    case 5:
        pos = vf();
        if (pos != -1)
...
int main()
{
    do
    {
        ...
        case 4:
            ...
            break;
        default:
            printf("Erreur, veuillez resélectionner");
        }
        system("cls");
        printf("\n\n");
    } while (a != 0);
